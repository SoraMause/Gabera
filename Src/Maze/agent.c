...
void setFastPathParameter1400( int8_t motion_buff[256], int8_t motion_data[256], uint8_t *cnt_motion, int8_t out_flag )
{
  ...
  if ( motion_queue[*cnt_motion-2] != SET_STRAIGHT ){
    ...
    fast_path[*cnt_motion-1].distance = 25.0f;
    fast_path[*cnt_motion-1].speed = 1000.0f;
...
void setFastPathParameterMax( int8_t motion_buff[256], int8_t motion_data[256], uint8_t *cnt_motion, int8_t out_flag )
{
  ...
  for( int i = 1; i < *cnt_motion; i++ ){
    ...
    if ( motion_queue[i] == SET_STRAIGHT ){
      ...
      } else if ( motion_queue[i+1] == DIA_CENTER_LEFT_135 || motion_queue[i+1] == DIA_CENTER_RIGHT_135 ){
        // 135度なら
        fast_path[i].end_speed = 1700.0f;
      } else {
      ...
      if ( motion_queue[i+1] == SET_DIA_STRAIGHT ){
        ...
        fast_path[i].speed = 1800.0f;
        fast_path[i].end_speed = 1700.0f;
      } else if ( motion_queue[i+1] == RETURN_DIA_LEFT_135 || motion_queue[i+1] == RETURN_DIA_RIGHT_135 ){
        // 135度復帰の時は速度1700に落とす
        fast_path[i].speed = 1800.0f;
        fast_path[i].end_speed = 1700.0f;
      } else {
